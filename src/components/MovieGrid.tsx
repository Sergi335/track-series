    ...
    <section className={'grid grid-cols-[repeat(auto-fill,minmax(210px,1fr))] gap-4 mx-auto'}>
      ...
      {series?.map((movie) => {
        ...
        return (
          <article
            key={movie.id}
            className={'movieItem grid relative transition-all duration-300 gap-4'}
            // style={{
          ...
          >
            <div className={`flex relative w-full ${isFollowing(movie.id) && isComplete(movie.id) ? 'border-red-600' : isFollowing(movie.id) ? 'border-blue-600' : isInWatchlist(movie.id) ? 'border-green-600' : 'border-transparent'} border-2 rounded-[32px]`}>
              <Dots className="absolute top-[10px] right-[10px] bg-[#0000007a] rounded-[100vw] z-10 cursor-pointer" onClick={() => handleItemClick(movie.id)} />
              {/* ImageCard */}
              <div className="rounded-[32px] overflow-hidden w-full">
            ...
            {isExpanded && (
              <div className="flex flex-col p-2 py-5 absolute top-0 right-0 w-full h-full bg-black/85">
